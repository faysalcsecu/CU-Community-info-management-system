...
app.put('/student/:id', upload.single('image'), (req, res) => {
  ...
  const query = `
      ...
      WHERE ST_ID = ?
  `;
  const values = [ST_Name, ST_Gmail, ST_Contact, ST_Batch, Dept_ID, pictureURL, studentId];
...
app.put('/teacher/:id', upload.single('image'), (req, res) => {
  ...
  const query = `
      ...
      WHERE T_ID = ?
  `;
  const values = [T_Name, T_Gmail, T_Contact, Dept_ID, C_ID, pictureURL, teacherId];
...
app.put('/staff/:id', upload.single('image'), (req, res) => {
  ...
  const query = `
      ...
      WHERE STF_ID = ?
  `;
  const values = [STF_Name, STF_Gmail, STF_Contact, Dept_ID, pictureURL, staffId];
